...
function TextInput({ label, name, control, errors, className, hasBg, passwordField, ...rest }) {
    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          type={passwordField && !isPasswordVisible ? 'password' :  'text'}
          className={`py-3   px-4 border border-gray-300 w-full
           rounded-md mt-1  ${errors[name] ? 'border-red-500' : 'focus:border-primary focus:ring-1'}  ${className} ${hasBg ? 'bg-inputBg' : ''}
           ${passwordField && 'pr-8'}
           `}
          {...field}
          {...rest}
